...
test('false "always exact location" claims are gone', () => {
  assert.ok(
...
test('no false "no categories" claim (event creation has a mandatory category field)', () => {
...
test('request-type events are described as approximate-until-accepted, not always exact', () => {
...
test('three event types are all present (open / request / private)', () => {
  assert.match(index, />Abierto</);
  assert.match(index, />Por solicitud</);
  assert.match(index, />Privado</);
});

test('Estados section exists, is human (no exact-location / real-time-tracking language)', () => {
...
test('the Estado -> Chat -> Event loop section exists and is compact (visual, not a wall of text)', () => {
...
test('Map section explains variable precision, not "see everything exactly"', () => {
...
test('NOW markers on the map use a distinct avatar shape, not a location pin', () => {
  const mapa = index.slice(index.indexOf('id="mapa"'), index.indexOf('id="personas"'));
  assert.match(mapa, /avatar-marker/, 'a NOW status is rendered as an avatar marker, not <div class="pin">');
});

test('attendance-privacy claim is opt-out framing, not an absolute "private" claim', () => {
...
test('nav reflects the current IA (Estados / Eventos / Personas / Privacidad)', () => {
...
test('hero ghost CTA points at a real section (no dangling #descubri anchor)', () => {
